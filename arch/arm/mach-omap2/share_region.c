...
/* <--LH_SWRD_CL1_ICS_Mervins@2012.06.05 :add device id*/
int ep_get_device_id(char *ver, const int len)
{
	if (!((g_share_region->device_id[0] == 'L' &&
	       g_share_region->device_id[1] == 'N')||
		  (g_share_region->device_id[0] == 'l' &&
	       g_share_region->device_id[1] == 'n')))
	{
		sprintf(ver, "LNVTABA2109AXXXX000000000000001\0");
		return -1;	
	}
	memcpy(ver, g_share_region->device_id, len);
	ver[(len-1)] = '\0';

	return 0;
}

EXPORT_SYMBOL(ep_get_device_id);
/* LH_SWRD_CL1_ICS_Mervins@2012.06.05--> */
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*LH-sam add for switch debug and gps*/
int get_debug_gps_sw(void)
{
	return (g_share_region->debug_gps_sw);
}

EXPORT_SYMBOL(get_debug_gps_sw);
