...
{
	printk(">>>1 evt_wifi_set_carddetect,kernel3.0, val: %d\n", val);
	
	evt_wifi_cd = val;
	if (wifi_status_cb)
		wifi_status_cb(val, wifi_status_cb_devid);
	else
		printk("%s: Nobody to notify...board_evt\n", __func__);
	
	printk("<<<2 evt_wifi_set_carddetect\n");
	return 0;
}
...
static int __init evt_wifi_init(void)
{
	...
	if (ret < 0) {
		pr_err(KERN_ERR "%s: can't reserve GPIO: %d\n", __func__,
			EVT_WIFI_ENPOW_GPIO);
