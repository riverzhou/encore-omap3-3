...
/**********max11871 touch**************/
#if defined(CONFIG_TOUCHSCREEN_MAX_I2C)
	#include <linux/max11871.h>
	#define MAX11871_GPIO 14 
	#define MAX11871_I2C_ADDR 0x48
/* Pins for the BeagleTouch OLED */
//	#define CS_PIN          139
//	#define MOSI_PIN        144
//	#define CLK_PIN         138
	#define RESET_PIN       157
...
#endif
/*LH_SWRD_CL1_Richard@2011.5.11 morgan touch for cl1  -->*/
/*<-- LH_SWRD_CL1_Richard@2011.9.1 ft5 touch for cl1 */
#if defined(CONFIG_TOUCHSCREEN_FT5_I2C)
#define FT5_IRQ_GPIO  14
#endif
/*LH_SWRD_CL1_Richard@2011.9.1 ft5 touch for cl1  -->*/
...
static struct regulator_consumer_supply evt_vmmc1_supply[] = {
	...
	REGULATOR_SUPPLY("vmmc_cam", "omap3isp"),
};
...
static struct regulator_init_data evt_vmmc1 = {
	...
	.num_consumer_supplies  = ARRAY_SIZE(evt_vmmc1_supply),
...
/* ICS LH_SWRD_CL1_Henry@2012.2.7 Add vdda_dac regulator with dev_name*/
/*
...
*/	
	.num_consumer_supplies		= ARRAY_SIZE(evt_vpll2_supply),
	.consumer_supplies		= evt_vpll2_supply,	
};
...
//&*&*&*SJ1_20110422, Modify mmcblk number.
#if defined (CONFIG_CHANGE_INAND_MMC_SCAN_INDEX)        
	evt_vmmc1_supply[0].dev = mmc[1].dev;
	evt_vsim_supply.dev = mmc[1].dev;
#else
	evt_vmmc1_supply[0].dev = mmc[0].dev;
	evt_vsim_supply.dev = mmc[0].dev;
#endif
//&*&*&*SJ2_20110422, Modify mmcblk number.


/* 20120211 Henry Add external card power control */
...
/**************************max11871 for touch****************************/
#if defined(CONFIG_TOUCHSCREEN_MAX_I2C)
static int max11871_report_ready(void)
{
	return !gpio_get_value(MAX11871_GPIO);
}

static int max11871_board_init(void)
{
	int gpio = MAX11871_GPIO;
	int ret = 0;

	ret = omap_mux_init_gpio(gpio, OMAP_PIN_INPUT_PULLUP);
	if (ret < 0) {
		printk(KERN_ERR "%s:%i: Failed to configure mux for GPIO %d \n",
	...
	if (ret < 0) {
		printk(KERN_ERR "%s:%i: Failed to request GPIO %d for IRQ\n",
			   __func__, __LINE__, gpio);
		return ret;
	}
...
static struct max11871_platform_data max11871_info = {
	.version   = 11871,
#if 0
	.abs_x_min = 34,
	.abs_x_max = 1502,
	...
	.abs_x_max = 600,//600,
	.abs_y_min = 20,//0,
	.abs_y_max = 1030,//1200,//1089,
	.abs_z_min = 0,
	.abs_z_max = 25000,

	//.abs_lcd_width  = 480,
	//.abs_lcd_height = 272,
	.abs_lcd_width  = 587,//613,//600,
	.abs_lcd_height = 1030,//1200,//1089,	
	.abs_touch_to_lcd_width_correction  = 1,
	.abs_touch_to_lcd_height_correction = 1,

//	.abs_button_area_x_max = 1800,
//	.abs_button_area_x_min = 1705,
	.abs_button_area_x_max = 466,//441,
	.abs_button_area_x_min = 134,//164,
//	.abs_button_area_y_max = 1200,
//	.abs_button_area_y_min = 0,
	.abs_button_area_y_max = 1089,//1085,
	.abs_button_area_y_min = 1050,//1062,
/*
	.abs_home_button_x   = 1760,
	.abs_home_button_y   = 1030,
	.abs_menu_button_x   = 1760,
	.abs_menu_button_y   =  777,
	.abs_back_button_x   = 1760,
	.abs_back_button_y   =  450,
	.abs_search_button_x = 1760,
	.abs_search_button_y =  175,
*/
	.abs_home_button_y   = 1070,//1085,
	.abs_home_button_x   = 293,//302,
	.abs_menu_button_y   = 1070,//1085,
	.abs_menu_button_x   = 174,//164,
	.abs_back_button_y   = 1070,//1085,
	.abs_back_button_x   = 426,//441,

	.abs_button_fuzz_x = 40,
	.abs_button_fuzz_y = 20,//50,

	.i2c_addr  = MAX11871_I2C_ADDR,
	.gpio_irq  = OMAP_GPIO_IRQ(MAX11871_GPIO),
	.gpio      = MAX11871_GPIO,
	.irq_flags = IRQF_TRIGGER_FALLING,
	.platform_hw_init = max11871_board_init,
	.get_report_ready = max11871_report_ready
};
...
#if defined(CONFIG_TOUCHSCREEN_MAX_I2C)
	{
		I2C_BOARD_INFO("max11871", MAX11871_I2C_ADDR),
		.irq = OMAP_GPIO_IRQ(MAX11871_GPIO),
		.platform_data = &max11871_info,
	},
#endif
...
/*<-- LH_SWRD_CL1_Richard@20110901  FT5 touch for CL1 */
#if defined(CONFIG_TOUCHSCREEN_FT5_I2C)
	{
	   I2C_BOARD_INFO("ft5x0x_ts", 0x38),
        .irq = OMAP_GPIO_IRQ(FT5_IRQ_GPIO),
	},
#endif
/*LH_SWRD_CL1_Richard@20110901 FT5 touch for CL1  -->*/
};
...
//	printk(KERN_INFO " i2c2 bus for Max11871 controller\n");//richard 20110720
	/*
...
int evt_cam_regulator(struct device *dev, int enable)
{
	static struct regulator *vmmc_cam = NULL;
	int ret = 0;

	if (vmmc_cam == NULL) {
		vmmc_cam = regulator_get(dev, "vmmc_cam");
		if (IS_ERR(vmmc_cam)) {
			vmmc_cam = NULL;
			return -ENODEV;
		}
	...
	if (enable) {
		if (regulator_is_enabled(vmmc_cam))
			return 0;

		ret = regulator_enable(vmmc_cam);
	} else {
		if (regulator_is_enabled(vmmc_cam))
			ret = regulator_disable(vmmc_cam);

		regulator_put(vmmc_cam);
		vmmc_cam = NULL;
	}
...
/**/
#ifdef CONFIG_TOUCHSCREEN_MAX_I2C
...
//	omap_mux_init_gpio(PANEL_PWR_PIN, OMAP_PIN_OUTPUT);
	omap_mux_init_gpio(MAX11871_GPIO, OMAP_PIN_INPUT_PULLUP);
