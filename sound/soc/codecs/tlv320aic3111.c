...
//  {DAC_CHN_REG, 0xD5},
  ...
  {LEFT_ANALOG_SPL, 0xa4}, //en plus 0x80 - 0x92
  {RIGHT_ANALOG_SPR, 0xa4},//en plus 0x80 - 0x92
...
static int set_input_source (struct snd_kcontrol *kcontrol,	struct snd_ctl_elem_value *ucontrol){
	...
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	int inputSource = 0;
	int gpio = AUDIO_CODEC_SPK_AMP_GPIO;
...
static void aic3111_capture_io_control(struct snd_soc_codec *codec, int on)
{
  ...
  { 
    ...
    aic3111_write(codec, ADC_FGA, 0x20);//0x00
    //aic3111_write(codec, HP_MIC_DECT, 0x86);
    aic3111_write(codec, MICBIAS_CTRL, 0x09);
    aic3111_write(codec, MICPGA_VOL_CTRL, 0x5a);
    aic3111_write(codec, MICPGA_MIN_CFG, 0x40);
   ...
   /*LHRDSW@Gavin,decrease the noise for record by filter-->*/
	printk("aic3111_capture_io_control=%d",on);
...
//&*&*&*BC2_110725: fix the issue that the device has no sounds for some applications when the device resume				
		...
		msleep(10);
		    ...
		    else
		    {
			  ...
			  aic3111_write(codec, CLASS_D_SPK ,0xC6 ); //ON 
			  aic3111_write(codec,SPL_DRIVER,0x0C);
			  aic3111_write(codec,SPR_DRIVER,0x0C);
			  //aic3111_write(codec, HPHONE_DRIVERS ,0x4); // OFF
...
/* Henry Li: Initialize TLV320AIC3111 to its default operating condition */
void aic3111_reset(void)
{
	int gpio=AUDIO_CODEC_RESET_GPIO;	//gpio103
...
static int aic3111_resume(struct snd_soc_codec *codec)
{
  ...
  int i;
  DBG("+ aic3111_resume\n");
  	aic3111_reset();	//Henry Li@2012.6.6 To fix noise after long-term suspend/resume process */
	/*<--LH_SWRD_BSP04@Gavin_20120510,fix bug for no sound when suspend for sometimes*/
	...
	aic3111_write(codec,CLK_REG_1,PLLCLK_2_CODEC_CLKIN);
	aic3111_write(codec,LEFT_ANALOG_HPL,0x8a);
	aic3111_write(codec,RIGHT_ANALOG_HPR,0x8a);
	aic3111_write(codec,LEFT_ANALOG_SPL,0xa4);
	aic3111_write(codec,RIGHT_ANALOG_SPR,0xa4);
	/*LH_SWRD_BSP04@Gavin_20120510,fix bug for no sound when suspend for sometimes-->*/
...
static ssize_t aic3111_aic_store(struct device * dev, struct device_attribute *attr, char * buf, size_t count)
{
	...
	if (error)
		return error;
	  printk("aic3111_aic_store  buf =%s\n", buf);
...
//&*&*&*BC1_110407:implement fm routeing
		...
		//aic3111_write(codec, HPHONE_DRIVERS, 0xC4); // ON
		aic3111_write(codec, CLASS_D_SPK, 0x06); // OFF
		printk("audio playback by headphone");
		aic3111_headset_driver_switch(1);
		...
		//aic3111_write(codec, HPHONE_DRIVERS ,0x4); // OFF
		aic3111_write(codec, CLASS_D_SPK ,0xC6 ); //ON
		printk("audio playback by speaker");
		}
//&*&*&*BC2_110407:implement fm routeing
	}		
...
static int aic3111_headset_detect(struct snd_soc_codec *codec)
{
  ...
  if(headset_present) {
	...
	aic3111_headset_driver_switch(0);//SWRD_LH@gavin_20111210,fix bug for music duan-continue when plugout headphone
